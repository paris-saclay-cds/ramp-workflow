...
restart_col_id = (len(_target_column_observation_names) +
                  len(_target_column_action_names))
cv = rw.cvs.KFoldPerEpisode(restart_name=restart_col_id)
get_cv = cv.get_cv

workflow = rw.workflows.FEGenRegNumpy(
    target_column_observation_names=_target_column_observation_names,
    max_n_components=_max_n_components,
...
def _read_data(path, X_name, data_label=''):
    ...
    # make sure that we have float for action because of check_cheat...
