...
complex_features = Hyperparameter(dtype="bool", default=True, values=[True, False])
    ...
    def transform(self, X_df):
        if int(complex_features):
            ...
            X_df_new = pd.concat(
                [
                    X_df.get(["Parch"]),
                    X_df.assign(LogFare=lambda x: 10 * np.log(x.Fare + 1.0) ** 0.5).get(
                        ["LogFare"]
                    ),
                    X_df.assign(
                        SibSp=lambda x: np.exp(x.SibSp) + 0.6 * np.exp(x.Parch)
                    ).get(["SibSp"]),
                    X_df.assign(Age=lambda x: 10 * np.log(x.Age + 0.01)).get(["Age"]),
                    pd.get_dummies(X_df.Sex, prefix="Sex", drop_first=False),
                    pd.get_dummies(X_df.Pclass, prefix="Pclass", drop_first=False),
