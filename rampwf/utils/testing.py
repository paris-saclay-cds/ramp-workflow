...
def assert_submission(ramp_kit_dir='./', ramp_data_dir='./',
    ...
    submission_name : str or a list of str, (default='starting_kit')
        The name of the submission to be tested. A list of submissions can be
        provided.
    ...
    if isinstance(submission_name, string_types):
        if submission_name == "*":
            ramp_submission_dir = join(ramp_kit_dir, 'submissions')
            submission_name = [directory
                               for directory in listdir(ramp_submission_dir)
                               if isdir(join(ramp_submission_dir, directory))]
        else:
            submission_name = [submission_name]

    for submission in submission_name:
