...
def train_test_submission(problem, module_path, X_train, y_train, X_test,
                          ...
                          output_path, trained_workflow_name, fold):
    ...
    is_pickle : boolean
        True if the trained workflow should be pickled
    ...
    output_path : str
        the path into which the trained workflow will be pickled
    save_output : boolean
        True if predictions should be written in files
    trained_workflow_name : str (default='trained_workflow.pkl')
        the file name of the pickled trained_workflow
    ...
    try:
        if is_partial_train:
            prev_trained_workflow = unpickle_trained_workflow(
                output_path, trained_workflow_name)
            if prev_trained_workflow is None:
                print("Can't unpickle trained workflow "
                      f'{output_path}/{trained_workflow_name}.')
            try:
    ...
    if is_pickle:
        is_pickled = pickle_trained_workflow(
            output_path, trained_workflow, trained_workflow_name,
...
def run_submission_on_full_train(problem, module_path, X_train, y_train,
    ...
    (y_pred_train, y_pred_test), _ = train_test_submission(
        problem, module_path, X_train, y_train, X_test, is_pickle,
        None, save_output, output_path, 'retrained_workflow.pkl', None)
...
def pickle_trained_workflow(fold_output_path, trained_workflow,
                            trained_workflow_name='trained_workflow.pkl',
                            is_silent=True, check_if_can_be_unpickled=True):
    ...
    """
    workflow_file = os.path.join(fold_output_path, trained_workflow_name)
    try:
    ...
    if check_if_can_be_unpickled and is_silent:
        trained_workflow_unpickled = unpickle_trained_workflow(
            fold_output_path, trained_workflow_name)
...
def unpickle_trained_workflow(fold_output_path,
                              trained_workflow_name='trained_workflow.pkl',
                              is_silent=True):
    ...
    trained_workflow_name : str (default='trained_workflow.pkl')
        the file name of the pickled workflow
    is_silent : boolean (default=True)
        whether return None when not pickleable or raise exception
    Returns
    -------
